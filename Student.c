...
void modify_student(Student* s) {
	...
	printf("Date of birth:\nDay: "); scanf("%d", &s->date_Of_Birth.day);
	printf("Month: "); scanf("%d", &s->date_Of_Birth.month);
	printf("Year: "); scanf("%d", &s->date_Of_Birth.year);
}
...
void pay_fine(Student* s) {
	...
	if (s->fine >= 0) {
		if (s->fine == 0) {
			...
			double money;
			printf("Enter money: "); scanf("%lf", &money);
			if (money >= s->fine) {
...
void request_book_issue(Student *s) {
	...
	int found = 0, flag = 0;
	printf("Enter the ISBN: "); scanf("%lld", &issued_ISBN);
